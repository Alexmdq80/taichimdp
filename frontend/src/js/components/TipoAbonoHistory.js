/**
 * TipoAbonoHistory Component
 * Displays the modification history of a tipo de abono
 */

import { displayApiError } from '../utils/errors.js';
import { makeRequest } from '../api/client.js';

export class TipoAbonoHistory {
  constructor(container, options = {}) {
    this.container = container;
    this.tipoAbono = options.tipoAbono;
    ...
    this.container.innerHTML = `
      <div class="card">
        <div class="card-header flex justify-between items-center">
          <h2 class="card-title">Historial: ${this.escapeHtml(this.tipoAbono.nombre)}</h2>
  ...
  async loadHistory() {
    ...
    try {
      const result = await makeRequest(`/tipos-abono/${this.tipoAbono.id}/history`, 'GET', null, true);
  ...
  renderHistory() {
    ...
    if (this.history.length === 0) {
      contentContainer.innerHTML = '<p class="text-center text-muted">No hay historial para este tipo de abono</p>';
      return;
    }
...
export default TipoAbonoHistory;
