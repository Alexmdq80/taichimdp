/**
 * PracticanteHistory Component
 * Displays the modification history of a Practicante
 */

import { displayApiError } from '../utils/errors.js';
import { makeRequest } from '../api/client.js';

export class PracticanteHistory {
  constructor(container, options = {}) {
    this.container = container;
    this.practicante = options.practicante;
    ...
    this.container.innerHTML = `
      <div class="card">
        <div class="card-header flex justify-between items-center">
          <h2 class="card-title">Historial: ${this.escapeHtml(this.practicante.nombre_completo)}</h2>
  ...
  async loadHistory() {
    ...
    try {
      const result = await makeRequest(`/practicantes/${this.practicante.id}/history`, 'GET', null, true);
  ...
  renderHistory() {
    ...
    if (this.history.length === 0) {
      contentContainer.innerHTML = '<p class="text-center text-muted">No hay historial para este practicante</p>';
      return;
    }
  ...
  renderChanges(item) {
    ...
    const fields = [
      ...
      { key: 'condiciones_medicas', label: 'Cond. Médicas' },
      { key: 'medicamentos', label: 'Medicamentos' },
      { key: 'limitaciones_fisicas', label: 'Limitaciones' },
      ...
      if (oldVal != newVal) {
        changes.push(`
          <li>
            ...
            <span class="text-danger strike-through">${this.formatValue(oldVal)}</span> &rarr; 
            <span class="text-success">${this.formatValue(newVal)}</span>
          </li>
        `);
      }
    });
  ...
  formatValue(val) {
    if (val === null || val === undefined || val === '') return '<em>vacio</em>';
...
export default PracticanteHistory;
