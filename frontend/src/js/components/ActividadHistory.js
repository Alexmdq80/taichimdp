/**
 * ActividadHistory Component
 * Displays the modification history of an Actividad
 */

import { displayApiError } from '../utils/errors.js';
import { makeRequest } from '../api/client.js';

export class ActividadHistory {
  constructor(container, options = {}) {
    this.container = container;
    this.actividad = options.actividad;
    ...
    this.container.innerHTML = `
      <div class="card">
        <div class="card-header flex justify-between items-center">
          <h2 class="card-title">Historial: ${this.escapeHtml(this.actividad.nombre)}</h2>
  ...
  async loadHistory() {
    ...
    try {
      const result = await makeRequest(`/actividades/${this.actividad.id}/history`, 'GET', null, true);
  ...
  renderHistory() {
    ...
    if (this.history.length === 0) {
      contentContainer.innerHTML = '<p class="text-center text-muted">No hay historial para esta actividad</p>';
      return;
    }
...
export default ActividadHistory;
